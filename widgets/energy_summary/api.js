...
module.exports = {
  ...
  async getEnergy({ homey, params, query }) {
    const deviceId = (params && params.deviceId) || (query && query.deviceId) || null;
    const period = String((query && query.period) || 'day').toLowerCase();
    const sort   = String((query && query.sort)   || 'desc').toLowerCase();
    ...
    channels.sort((a, b) => {
      ...
      const ea = a.energy != null ? a.energy : -Infinity;
      const eb = b.energy != null ? b.energy : -Infinity;
