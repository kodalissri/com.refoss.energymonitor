...
class RefossApi {
  ...
  async getEmStatus() {
    // Try the namespace the device actually supports.
    // Official aiorefoss library uses Refoss.Status.Get which bundles
    // all component status (including "em" array) in one call.
    // Fall back to the component-level Em.Status.Get if needed.
    try {
      return await this._get('Refoss.Status.Get');
    } catch (err) {
  ...
  static parseEmStatus(raw) {
    ...
    // Refoss.Status.Get wraps channel data under result.em (array)
    // Em.Status.Get wraps it under result.status (array)
    // Support both so either method works transparently.
    const statusArray = result.em || result.status || [];
    ...
    const temp = result.temperature !== undefined
      ? result.temperature
      : (result.sys && result.sys.temperature !== undefined
          ? result.sys.temperature
          : null);
