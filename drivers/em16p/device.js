...
class Em16pDevice extends Homey.Device {
  ...
  async onInit() {
    ...
    if (this._isChannelDevice) {
      ...
      this.log(`Em16p channel snapshot ${this._channelKey}: available=${this.getAvailable()} caps=${JSON.stringify(capabilities)}`);
      ...
      for (const cap of channelCapsToEnsure) {
        if (!this.hasCapability(cap)) {
          this.log(`Em16p channel ${this._channelKey} addCapability ${cap}`);
          await this.addCapability(cap).catch(() => {});
        }
      }
      if (this.hasCapability('measure_temperature')) {
        this.log(`Em16p channel ${this._channelKey} removeCapability measure_temperature`);
      ...
      // connection storm: 18 simultaneous HTTP requests overwhelm the device.
  ...
  async _setupWebhook() {
    ...
    try {
      ...
      this.log(`Registering webhook: event=${emEvent} url=${webhookUrl}`);
      const regResult = await this._api.registerHomeyWebhook(webhookUrl, emEvent, {
        channelIds: RefossApi.EM16P_CHANNELS.map(ch => ch.id),
      });
  ...
  async _resolveEmEvent() {
    try {
      ...
      // em.* events are per-channel and require cid; handled in registerHomeyWebhook.
      const preferred = [
  ...
  async _onWebhookData(data) {
    ...
    if (!this.getAvailable()) {
      this.log(`Em16p channel ${this._channelKey} restoring availability after webhook data`);
  ...
  async _pollDevice() {
    try {
      ...
      if (this._isChannelDevice) {
        ...
        if (!this.getAvailable()) {
          this.log(`Em16p channel ${this._channelKey} restoring availability after poll`);
      ...
      // Main device shows whole-home totals from the two mains CTs only:
      //   A1 (channel id 1) and B1 (channel id 7) are clamped to the panel feed lines.
      // All other CTs are branch circuits and are shown only on their channel devices.
      const MAINS_IDS = [1, 7]; // A1, B1
      const chA1 = statusMap[1];
      const chB1 = statusMap[7];

      const mainsPower   = MAINS_IDS.reduce((s, id) => s + (statusMap[id] && statusMap[id].power   != null ? statusMap[id].power   : 0), 0);
      const mainsCurrent = MAINS_IDS.reduce((s, id) => s + (statusMap[id] && statusMap[id].current != null ? statusMap[id].current : 0), 0);
      const mainsHasPower   = MAINS_IDS.some(id => statusMap[id] && statusMap[id].power   != null);
      const mainsHasCurrent = MAINS_IDS.some(id => statusMap[id] && statusMap[id].current != null);

      if (mainsHasPower)   updates.push(this._updateCapability('measure_power',   mainsPower));
      if (mainsHasCurrent) updates.push(this._updateCapability('measure_current', mainsCurrent));

      // Voltage and PF from A1 (both feed lines share the same split-phase voltage)
      if (chA1 && chA1.voltage != null)
        updates.push(this._updateCapability('measure_voltage', chA1.voltage));
      if (chA1 && chA1.pf != null)
        updates.push(this._updateCapability('measure_power_factor', chA1.pf));

      // Dispatch all channels so channel devices update
      ...
      // Energy totals for main device = sum of A1 + B1 mains CTs only
      ...
      if (statusMap.temperature != null)
        updates.push(this._updateCapability('measure_temperature', statusMap.temperature));
      ...
      if (!this._loggedFirstMainPoll) {
        ...
        this.log(`First main poll applied (mains_power=${mainsPower}, A1_power=${(chA1 && chA1.power)}, B1_power=${(chB1 && chB1.power)}, temp=${statusMap.temperature}, sys=${sysSnippet})`);
      }
  ...
  async onAdded() {
    this.log(`Em16pDevice added: ${this.getName()}`);
  }
